# :material-package-variant-closed: Versiones

!!! abstract "Seguimiento del trabajo por versión"
    Cada versión tiene un **objetivo claro**, un conjunto de **funcionalidades definidas** y el **estado de avance** de cada una. Esta página centraliza la vista de todas las versiones, activas y cerradas.

---

## :material-progress-clock: Versiones activas

<div class="grid cards" markdown>

-   :material-circle:{ style="color: #f59e0b" } **Versión 001 — Programa Becas y RBAC**

    ---

    :material-calendar-range: **Período:** 3 jun → 18 jun 2026
    :material-bullseye-arrow: **Objetivo:** Definir alcance funcional de Programa Becas y desarrollo del motor RBAC
    :material-progress-check: **Estado:** En progreso

    [:octicons-arrow-right-16: Ver detalle de la versión](version-001.md)

</div>

---

## :material-archive-outline: Versiones cerradas

!!! info "Aún no hay versiones cerradas"
    Las versiones completadas aparecerán acá con su resumen de resultados, ítems completados y postergados.

| Versión | Período | :material-check-circle-outline: Completado | :material-clock-time-eight-outline: Postergado |
|---|---|---|---|
| — | — | — | — |

---

## :material-information-outline: Cómo se organizan las versiones

<div class="grid cards" markdown>

-   :material-timer-outline: **Duración variable**

    Se acuerda con el cliente según la complejidad del alcance.

-   :material-clipboard-list-outline: **Alcance comprometido**

    Se define al comenzar cada versión con los ítems priorizados del backlog.

-   :material-lock-outline: **Foco protegido**

    No se aceptan cambios de alcance dentro de la versión en curso.

-   :material-presentation-play: **Revisión al cierre**

    Demo de lo entregado, validación de resultados y planificación del siguiente.

</div>

[:material-arrow-right: Ver metodología completa](../methodology.md){ .md-button }
