# :material-cash-multiple: Módulo Financiero

!!! abstract "Seguimiento de presupuesto y consumo"
    Vista consolidada mensual del presupuesto y las horas consumidas. Permite al cliente tener visibilidad centralizada del estado financiero del proyecto.

---

## :material-progress-clock: Mes en curso

<div class="grid cards" markdown>

-   :material-calendar-clock: **Septiembre 2026**

    ---

    :material-wallet-outline: Presupuesto: **700 horas**<br>
    :material-clock-check-outline: Consumido: **413 h 00 min** (59%) — al 10/09<br>
    :material-check-circle-outline: Saldo: **287 h 00 min** disponibles<br>
    :material-swap-horizontal: Incluye el traslado del excedente de agosto

    **Estado:** :material-circle:{ style="color: #3b82f6" } En curso

    [:octicons-arrow-right-16: Ver detalle del mes](mes-2026-09.md)

</div>

---

## :material-archive-check-outline: Meses cerrados

<div class="grid cards" markdown>

-   :material-calendar-check: **Agosto 2026**

    ---

    :material-wallet-outline: Presupuesto: **700 horas**  
    :material-clock-check-outline: Imputado: **700 horas** (100%)  
    :material-progress-clock: Esfuerzo real: **803 h 30 min**  
    :material-swap-horizontal: Trasladado a septiembre: **103 h 30 min**

    **Estado:** :material-circle:{ style="color: #10b981" } Cerrado al 100% del presupuesto

    [:octicons-arrow-right-16: Ver resumen del mes](mes-2026-08.md)

-   :material-calendar-check: **Julio 2026**

    ---

    :material-wallet-outline: Presupuesto: **500 horas**  
    :material-clock-check-outline: Consumido: **500 horas** (100%)  
    :material-check-circle-outline: Saldo: **0 horas**

    **Estado:** :material-circle:{ style="color: #10b981" } Cerrado al 100% del presupuesto

    [:octicons-arrow-right-16: Ver resumen del mes](mes-2026-07.md)

-   :material-calendar-check: **Junio 2026**

    ---

    :material-wallet-outline: Presupuesto: **500 horas**  
    :material-clock-check-outline: Consumido: **499 horas** (100%)  
    :material-check-circle-outline: Saldo: **1 hora** sin consumir

    **Estado:** :material-circle:{ style="color: #10b981" } Cerrado dentro del presupuesto

    [:octicons-arrow-right-16: Ver resumen del mes](mes-2026-06.md)

</div>

---

[:material-home: Volver al inicio](../index.md){ .md-button }
