...
| 63 | El login tarda por el hash de la contraseña y el HTTP corre en un solo proceso | Transversal / login e infraestructura de ejecución | `#sesion` `#infra` | PM — en sesión: «noto que la carga de algunas pantallas tardan más de lo común, ejemplo el login» y «vamos con tema desarrollo y armá una rama para este cambio» | 03/09/2026 | 🟢 **Hecho — cerrado por el PM el 16/09/2026; el modo gunicorn queda como configuración opcional del ambiente** | No requiere |
...
- **11/09/2026 — `development` mergeado en la rama y la rama al ambiente QA de ECOM.** Por pedido del PM, la
  ...
  **Cambio 74**, con las referencias del código renumeradas. (6) Tailwind recompilado; el `.gitlab-ci.yml` es el de
...
🟢 **HECHO — 16/09/2026.** El hash Argon2 quedó integrado a `development` y viajó en el release a `main`, así
que ya está en el circuito de ECOM. El **modo gunicorn** no se da por pendiente: queda disponible como
**configuración opcional del ambiente** (`APP_RUNTIME`), a decisión de la infraestructura del organismo.
El PM cierra el cambio el 16/09/2026 sin esperar esa definición, y **sin validación del Ministerio**: es
trabajo técnico interno, no una funcionalidad que el cliente apruebe.
...
| **Migración** | `programas.0066_siis_envio_beneficiarios` |
...
- `programas/migrations/0066_siis_envio_beneficiarios.py`.
...
Migración `programas.0066_siis_envio_beneficiarios`: tabla nueva `EnvioSIIS`, más cuatro
...
Revertir los commits de la rama y aplicar la migración inversa de `programas.0066`: se
...
- **14/09/2026 — migración renumerada de `0060` a `0066`.** Al llevar el cambio a
  `feature/constructor-formularios` (lo que se prueba en testing), el `0060` ya lo ocupaba
  `catalogo_grupos_origen_canal`; la migración del alta pasa a depender de
  `0065_padron_relevamiento_herencia`. Con el merge del 15/09/2026 que trajo el constructor a
  `development`, el `0066` es el número definitivo en todas las ramas.

---

# Cambio 74 — Padrón con herencia: convocatoria → relevamientos, con padrón propio por relevamiento
...
- **11/09/2026 — Renumerado de 59 a 72.** Al mergear `development` en la rama, el número 59 ya lo ocupaba «El
  link público muestra el contacto del programa». Esta entrada y las referencias del código (`Cambio 74`) se
  renumeraron; la migración es ahora `programas.0065_padron_relevamiento_herencia`.
- **14/09/2026 — Renumerado de 72 a 74.** Al mergear `development` en la rama, el 72 ya lo ocupaba la
...
# Cambio 75 — El link público se presenta como «Programa +Más Futuro» y el rechazo por padrón deriva a Soporte Técnico
