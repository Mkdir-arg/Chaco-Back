...
class ProgramaDetailView(CapacidadRequeridaMixin, LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if programa.tipo in ["NACHEC", "ÑACHEC"]:
            ...
            derivaciones_qs = DerivacionPrograma.objects.filter(programa_destino=programa).select_related(
                "ciudadano", "programa_origen", "derivado_por"
            )
            ...
            # Filtros activos
            context["filtros_activos"] = {"estado": estado_filtro, "urgencia": urgencia_filtro, "busqueda": busqueda}
            ...
            tasa_aceptacion = round(
                (derivaciones_aceptadas / derivaciones_totales * 100) if derivaciones_totales > 0 else 0, 1
            )

            # Fase 2: Asignación
            relevamientos_completados = RelevamientoNachec.objects.filter(
                caso__in=casos_nachec, completado=True
            ).count()
            ...
            prestaciones_con_sla = prestaciones_qs.filter(
                estado="ENTREGADA", sla_hasta__isnull=False, fecha_entregada__isnull=False
            )
            cumplidas = sum(
                1
                for p in prestaciones_con_sla
                if p.fecha_entregada
                and p.sla_hasta
                and timezone.make_aware(timezone.datetime.combine(p.fecha_entregada, timezone.datetime.min.time()))
                <= p.sla_hasta
            )
            ...
            prestaciones_qs = (
                PrestacionNachec.objects.filter(
                    Q(responsable=self.request.user) | Q(caso__coordinador=self.request.user)
                )
                .select_related("caso__ciudadano_titular", "responsable", "plan")
