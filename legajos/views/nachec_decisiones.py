...
def _confirmar_evaluacion(request, caso, relevamiento, tarea_evaluacion):
    ...
    with transaction.atomic():
        ...
        tarea_evaluacion.resultado = (
            f"Evaluado. Categoría: {categoria_final}. Plan definido con {len(componentes)} componentes."
        )
        tarea_evaluacion.save()

        tarea_activacion_existente = TareaNachec.objects.filter(
            caso=caso, tipo="OTRO", titulo__icontains="Activar plan", estado__in=["PENDIENTE", "EN_PROCESO"]
        ).first()
...
def _rechazar_caso(request, caso, tarea_evaluacion):
    ...
    with transaction.atomic():
        ...
        TareaNachec.objects.filter(caso=caso, estado__in=["PENDIENTE", "EN_PROCESO"]).exclude(
            id=tarea_evaluacion.id
        ).update(estado="CANCELADA", resultado=f"Cancelada por rechazo del caso: {motivo}")
...
def activar_plan(request, caso_id):
    ...
    if request.method == "GET":
        ...
        if plan.incluye_alimentacion:
            componentes.append(
                {"codigo": "ALIMENTARIA", "nombre": "Alimentación", "icono": "utensils", "color": "orange"}
            )
        ...
        if plan.incluye_educacion:
            componentes.append(
                {"codigo": "EDUCACION", "nombre": "Educación", "icono": "graduation-cap", "color": "purple"}
            )
        if plan.incluye_empleo:
            componentes.append(
                {"codigo": "EMPLEO", "nombre": "Empleo/Capacitación", "icono": "briefcase", "color": "green"}
            )
        if plan.incluye_emprendimiento:
            componentes.append(
                {"codigo": "EMPRENDIMIENTO", "nombre": "Emprendimiento", "icono": "lightbulb", "color": "yellow"}
            )
        ...
        with transaction.atomic():
            ...
            for comp_data in componentes_data:
                ...
                tarea_existente = TareaNachec.objects.filter(
                    caso=caso, prestacion=prestacion, tipo="ENTREGA", estado__in=["PENDIENTE", "EN_PROCESO"]
                ).first()
...
{comp_data["observaciones"]}""",
            ...
            tarea_activacion.resultado = (
                f"Plan activado. {prestaciones_creadas} prestaciones creadas, {tareas_creadas} tareas generadas."
            )
...
Tareas: {tareas_creadas} generadas""",
        ...
        messages.success(
            request,
            f"Plan activado exitosamente. Se crearon {prestaciones_creadas} prestaciones y {tareas_creadas} tareas de ejecución.",
        )
        return redirect("legajos:programa_detalle", pk=2)
