...
def asignar_territorial(request, caso_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Territorial asignado exitosamente. Tarea de relevamiento creada con vencimiento {fecha_limite_obj.strftime('%d/%m/%Y')}.",
        )
        return redirect("legajos:programa_detalle", pk=2)
...
def iniciar_relevamiento(request, caso_id):
    ...
    if request.method == "POST":
        ...
        if result["sla_vencido"]:
            messages.warning(
                request, "Relevamiento iniciado. ADVERTENCIA: SLA vencido - el inicio se registró fuera de término."
            )
        else:
...
def finalizar_relevamiento(request, caso_id):
    ...
    try:
        ...
        messages.error(
            request,
            "No existe relevamiento sin completar para este caso. Debe completar el formulario de relevamiento primero.",
        )
        return redirect("legajos:nachec_formulario_relevamiento", caso_id=caso.id)
    ...
    if request.method == "POST":
        ...
        if riesgo_critico and adjuntos.count() == 0:
            messages.error(
                request,
                "No se puede finalizar: debe adjuntar evidencia por situación de riesgo crítico (urgencia alimentaria, violencia o vivienda precaria/calle)",
            )
            return redirect("legajos:nachec_finalizar_relevamiento", caso_id=caso.id)
        ...
        with transaction.atomic():
            ...
            if tarea_relevamiento:
                ...
                tarea_relevamiento.resultado = (
                    f"Relevamiento finalizado. Score: {score_total} ({score_categoria}). Versión: v1"
                )
...
Observaciones: {observaciones_cierre[:100]}...""",
        ...
        messages.success(
            request, f"Relevamiento finalizado. Caso pasó a EVALUADO. Score: {score_total} ({score_categoria})"
        )
        return redirect("legajos:programa_detalle", pk=2)
