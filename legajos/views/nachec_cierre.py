...
def cerrar_caso_nachec(request, caso_id):
    ...
    if request.method == "GET":
        ...
        ultima_prestacion = (
            PrestacionNachec.objects.filter(caso=caso, estado="ENTREGADA").order_by("-fecha_entregada").first()
            if plan_vigente
            else None
        )

        evaluacion = caso.evaluacion if hasattr(caso, "evaluacion") else None
        prestaciones_entregadas = PrestacionNachec.objects.filter(caso=caso, estado="ENTREGADA").count()
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            tareas_abiertas = TareaNachec.objects.filter(caso=caso, estado__in=["PENDIENTE", "EN_PROCESO"])
            tareas_canceladas = tareas_abiertas.count()
            tareas_abiertas.update(estado="CANCELADA", resultado=f"Cancelada por cierre de caso: {motivo_cierre}")
...
def reabrir_caso_nachec(request, caso_id):
    ...
    if request.method == "GET":
        ...
        prestaciones_activas = PrestacionNachec.objects.filter(
            caso=caso, estado__in=["PROGRAMADA", "EN_PROCESO", "EN_CURSO"]
        ).count()
        evaluacion = caso.evaluacion if hasattr(caso, "evaluacion") else None
