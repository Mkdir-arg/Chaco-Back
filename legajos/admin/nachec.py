...
class CasoNachecAdmin(admin.ModelAdmin):
    ...
    search_fields = [
        "ciudadano_titular__nombre",
        "ciudadano_titular__apellido",
        "ciudadano_titular__dni",
        "municipio",
        "localidad",
    ]
    ...
    fieldsets = (
        ("Ciudadano", {"fields": ("ciudadano_titular",)}),
        ("Estado", {"fields": ("estado", "prioridad", "tiene_duplicado", "doc_pendiente")}),
        ("Ubicación", {"fields": ("municipio", "localidad", "direccion", "referencias_domicilio")}),
        ("Asignaciones", {"fields": ("operador_admision", "coordinador", "territorial", "referente_programa")}),
        (
            "Fechas",
            {
                "fields": (
        ...
        ("Motivos", {"fields": ("motivo_derivacion", "motivo_rechazo", "motivo_suspension", "motivo_cierre")}),
        ("SLA", {"fields": ("sla_revision", "sla_relevamiento")}),
    ...
    def prioridad_badge(self, obj):
        colors = {"BAJA": "#10b981", "MEDIA": "#f59e0b", "ALTA": "#ef4444", "URGENTE": "#dc2626"}
...
class RelevamientoNachecAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Caso", {"fields": ("caso", "territorial")}),
        (
            "Composición Familiar",
            {"fields": ("cantidad_convivientes", "hay_embarazo", "hay_discapacidad", "detalle_discapacidad")},
        ),
        ("Ingresos y Empleo", {"fields": ("ingreso_mensual_rango", "fuente_ingreso", "situacion_laboral")}),
        (
            "Vivienda",
            {
                "fields": (
        ...
        ("Salud", {"fields": ("cobertura_salud", "condiciones_cronicas")}),
        ("Educación", {"fields": ("menores_escolarizados", "motivo_no_escolarizacion")}),
        ("Alimentación", {"fields": ("acceso_alimentos", "frecuencia_comidas")}),
        ("Riesgos", {"fields": ("hay_violencia", "detalle_violencia", "hay_situacion_calle", "urgencia_alimentaria")}),
        ("Geolocalización", {"fields": ("latitud", "longitud")}),
        ("Estado", {"fields": ("completado", "fecha_finalizacion", "observaciones")}),
    )
...
class EvaluacionVulnerabilidadAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Caso", {"fields": ("caso", "relevamiento", "evaluador")}),
        ("Scoring", {"fields": ("score_total", "score_version", "categoria_sugerida")}),
        ("Dictamen", {"fields": ("categoria_final", "dictamen", "override_categoria", "justificacion_override")}),
    )
...
class PlanIntervencionNachecAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Caso", {"fields": ("caso", "referente")}),
        ("Plan", {"fields": ("objetivo_general", "fecha_inicio", "horizonte_dias", "vigente")}),
        (
            "Componentes",
            {
                "fields": (
...
class PrestacionNachecAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Prestación", {"fields": ("plan", "caso", "tipo", "subtipo", "descripcion")}),
        ("Estado", {"fields": ("estado", "frecuencia")}),
        ("Fechas", {"fields": ("fecha_programada", "fecha_entregada")}),
        ("Responsable", {"fields": ("responsable", "lugar_entrega")}),
        (
            "Confirmación",
...
class TareaNachecAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Tarea", {"fields": ("caso", "prestacion", "tipo", "titulo", "descripcion")}),
        ("Asignación", {"fields": ("asignado_a", "creado_por")}),
        ("Estado", {"fields": ("estado", "prioridad")}),
        ("Fechas", {"fields": ("fecha_vencimiento", "fecha_completada")}),
...
class SeguimientoTerritorialAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Seguimiento", {"fields": ("caso", "territorial", "tipo", "fecha_seguimiento")}),
        ("Resultado", {"fields": ("resultado", "cambios_detectados", "proxima_accion", "fecha_proxima_revision")}),
        ("Evidencias", {"fields": ("fotos", "observaciones")}),
    )
