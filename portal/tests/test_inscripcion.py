...
def _tolerar_render_local(exc):
    ...
    escondió acá (revisión Cambio 41)."""
...
class ServiciosInscripcionTests(_BaseInscripcionTest):
    ...
    def test_rate_limit_por_ip(self):
        ...
        for _ in range(servicio.MAX_INTENTOS_IP):
            self.assertFalse(
                rate_limit_excedido(request, "inscripcion_paso1", servicio.MAX_INTENTOS_IP, servicio.VENTANA_SEGUNDOS)
            )
        self.assertTrue(
            rate_limit_excedido(request, "inscripcion_paso1", servicio.MAX_INTENTOS_IP, servicio.VENTANA_SEGUNDOS)
        )
        ...
        self.assertFalse(
            rate_limit_excedido(otra_ip, "inscripcion_paso1", servicio.MAX_INTENTOS_IP, servicio.VENTANA_SEGUNDOS)
        )
...
class Paso1FlujoTests(_BaseInscripcionTest):
    ...
    def test_rate_limit_corta_sin_consultar(self, mock_consulta):
        with (
            ...
            patch("portal.views.inscripcion.paso1_excedido", return_value=True),
        ):
            try:
                self._post_paso1()
            except AttributeError as exc:
