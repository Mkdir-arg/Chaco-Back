...
def get_ciudadano_perfil_context(*, user, ciudadano):
    ...
    if hasattr(ciudadano, "inscripciones_programas"):
        inscripciones_activas = list(
            ciudadano.inscripciones_programas.filter(estado__in=["ACTIVO", "EN_SEGUIMIENTO"]).select_related(
                "programa"
            )[:5]
        )
    ...
    if hasattr(ciudadano, "inscripciones_programas"):
        inscripciones_timeline = ciudadano.inscripciones_programas.select_related("programa").order_by(
            "-fecha_inscripcion"
        )[:3]
...
def get_ciudadano_programas_context(ciudadano):
    return {
        ...
        "inscripciones_historial": ciudadano.inscripciones_programas.filter(estado__in=["CERRADO", "SUSPENDIDO"])
        .select_related("programa")
        .order_by("-fecha_inscripcion"),
    }
...
def get_ciudadano_programa_derivaciones(ciudadano, programa):
    return (
        DerivacionPrograma.objects.filter(
            ciudadano=ciudadano,
            programa_destino=programa,
        )
        .select_related("programa_origen", "programa_destino")
        .order_by("-creado")
    )
