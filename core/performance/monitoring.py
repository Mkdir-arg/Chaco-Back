...
class SystemMonitor:
    ...
    def collect_application_metrics(self):
        ...
        try:
            ...
            user_stats = User.objects.aggregate(
                total=Count("id"), active_today=Count("id", filter=Q(last_login__gte=hace_24h))
            )

            ciudadano_stats = Ciudadano.objects.aggregate(
                total=Count("id"), created_today=Count("id", filter=Q(creado__gte=hoy_inicio))
            )

            conversacion_stats = Conversacion.objects.aggregate(
                total=Count("id"), active=Count("id", filter=Q(estado="ACTIVA"))
            )
    ...
    def _get_online_users(self):
        ...
        try:
            ...
            return User.objects.filter(last_login__gte=timezone.now() - timedelta(minutes=5)).count()
        except Exception as e:
            import logging
