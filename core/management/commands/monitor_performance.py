    ...
    def handle(self, *args, **options):
        ...
        while time.time() - start_time < duration:
            ...
            if query_count > threshold:
                ...
                self.stdout.write(self.style.WARNING(f"Alert: {query_count} queries in 5s (threshold: {threshold})"))

                if analysis["n1_detected"]:
                    self.stdout.write(self.style.ERROR(f"N+1 detected: {analysis['similar_queries']} similar queries"))
