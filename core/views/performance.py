        ...
        response_data = {
            ...
            "partitioning": {
                "active_partitions": sum(
                    stats.get("partition_count", 0) for stats in phase2_stats.get("partitioning", {}).values()
                ),
                "total_size_mb": sum(
                    stats.get("total_size_mb", 0) for stats in phase2_stats.get("partitioning", {}).values()
                ),
            },
            "query_optimization": {
                "queries_analyzed": phase2_stats.get("query_optimization", {})
                .get("metrics", {})
                .get("total_queries_analyzed", 0),
                "slow_queries": phase2_stats.get("query_optimization", {})
                .get("metrics", {})
                .get("slow_queries_count", 0),
                "optimization_opportunities": len(
                    phase2_stats.get("query_optimization", {})
                    .get("recommendations", {})
                    .get("frequent_optimizations", [])
                ),
            },
            "connection_pool": {
                "total_connections": phase2_stats.get("connection_pool", {})
                .get("global_stats", {})
                .get("total_connections", 0),
                "active_connections": phase2_stats.get("connection_pool", {})
                .get("global_stats", {})
                .get("active_connections", 0),
                "avg_response_time": phase2_stats.get("connection_pool", {})
                .get("global_stats", {})
            ...
            "indexing": {
                ...
                "high_priority_suggestions": len(
                    [
                        idx
                        for idx in phase2_stats.get("indexing", {}).get("recommended_indexes", [])
...
def run_phase2_tests_api(request):
    ...
    try:
        ...
        results = {"timestamp": timezone.now().isoformat(), "tests_executed": [], "summary": {}, "recommendations": []}
            ...
            results["recommendations"].append(
                {
                    ...
                    "message": "OPTIMIZAR: Usar .select_related() en queries de LegajoAtencion",
