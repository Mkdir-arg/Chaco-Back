...
class HistorialAsignacion(models.Model):
    ...
    operador_anterior = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="asignaciones_anteriores"
    )
    operador_nuevo = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="asignaciones_nuevas"
    )
    usuario_que_asigna = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="asignaciones_realizadas"
    )
...
class ColaAsignacion(models.Model):
    ...
    def actualizar_contador(self):
        ...
        self.conversaciones_actuales = Conversacion.objects.filter(
            operador_asignado=self.operador, estado="activa"
        ).count()
        self.save()
...
class MetricasOperador(models.Model):
    ...
    def actualizar_metricas(self):
        ...
        stats = Conversacion.objects.filter(operador_asignado=self.operador).aggregate(
            total=Count("id"),
